...
const Login = () => {
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      toast({
        title: "Welcome back!",
        description: "You're connected — let's get moving.",
      });
      navigate("/");
    } catch (err) {
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] flex items-center justify-center px-4">
      <div className="w-full max-w-md p-6 rounded-2xl border border-border bg-card/60 backdrop-blur-xl animate-in fade-in slide-in-from-top-4 duration-700">
        ...
        <p className="text-center text-muted-foreground mb-6">Log in to continue your journey</p>
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div>
            ...
            <Input id="height" type="number" value={height} onChange={(e) => setHeight(e.target.value)} required placeholder="175" />
          </div>
          <div>
            <Label htmlFor="weight">Weight (kg)</Label>
            <Input id="weight" type="number" value={weight} onChange={(e) => setWeight(e.target.value)} required placeholder="70" />
          </div>
          <div>
